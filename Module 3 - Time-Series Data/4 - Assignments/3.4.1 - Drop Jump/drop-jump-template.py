...
def main(full_path_to_file):
    ...
    baseline_length = 20 # setting baseline length to first 20 points

    # over the baseline, determine the average signal value
    baseline = np.mean(force_plate[:baseline_length]) # baseline of first 20 points of data
    ...
    for index in range(baseline_length, len(force_plate_list)):
        ...
        if value > baseline + delta:
            ...
            first_landing_index = index # finding first landing when comparing baseline + delta to a certain value
    ...
    for index in range(first_landing_index + 10, len(force_plate_list)):
        ...
        value = force_plate_list[index] # your code here
        if value < baseline + delta:
...
if __name__ == "__main__":
    ...
    path_to_data_folder = "C:\\Users\\austi\\OneDrive\\Documents\\GitHub\\ENGR315-sp2026-student\\data\\drop-jump\\force-plate\\"
